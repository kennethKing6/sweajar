...
describe("AddSwearType", () => {
  ...
  it("renders without crashing", async () => {
    await act(async () => await render(<AddSwearType />));
  });

  it("updates state when input fields are changed", async () => {
    const { getByLabelText } = await act(
      async () => await render(<AddSwearType />),
    );
    await act(async () => {
      fireEvent.change(getByLabelText("Enter Violation Name"), {
        target: { value: "Test Name" },
      });
      fireEvent.change(getByLabelText("Enter Violation Description"), {
    ...
    await waitFor(() => {
      ...
      expect(getByLabelText("Enter Violation Description").value).toBe(
        "Test Description",
      );
    });
  });
  ...
  it("calls onAdd with new swear type when form is submitted", async () => {
    ...
    const { getByText, getByLabelText } = await act(
      async () => await render(<AddSwearType onAdd={onAdd} />),
    );
  ...
  it("shows alert when form is submitted with invalid inputs", async () => {
    const { getByText } = await act(async () => await render(<AddSwearType />));

    fireEvent.click(getByText("Submit"));

    await waitFor(() => {
      expect(window.alert).toHaveBeenCalledWith(
        "Please enter name and description for the new swear type.",
      );
    });
  });
});
